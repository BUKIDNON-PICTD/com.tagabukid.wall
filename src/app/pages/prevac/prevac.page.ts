...
import { PrevacService } from './../../services/prevac.service';
...
@Component({
  selector: 'app-prevac',
  templateUrl: './prevac.page.html',
  styleUrls: ['./prevac.page.scss'],
})
export class PrevacPage implements OnInit {
  ...
  saveprofile() {
    ...
    try {
      ...
      profiletoadd.address_barangay_lguname = this.barangays.find(
        o => o.code === profiletoadd.barangay.code
      ).lguname;

      profiletoadd.address_municipality_lguname = this.municipalities.find(
        o => o.code === profiletoadd.municipality.code
      ).lguname;

      profiletoadd.address_province_lguname = this.provinces.find(
        o => o.code === profiletoadd.province.code
      ).lguname;

      profiletoadd.address_barangay_code = profiletoadd.barangay.code;
      profiletoadd.address_municipality_code = profiletoadd.municipality.code;
      profiletoadd.address_province_code = profiletoadd.province.code;
      profiletoadd.address_street = profiletoadd.street;
      ...
      if (this.mode === "create"){
        ...
        this.prevacsvc.addItem(profiletoadd).then(item => {
          this.presentAlert(item.scheddate);
